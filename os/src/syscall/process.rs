...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let user_buffer_ptr = ts as *const u8;
    let user_buffer = translated_byte_buffer(user_token, user_buffer_ptr, size_of::<TimeVal>());
    ...
    for slice in user_buffer {
        slice.copy_from_slice(&time_bytes[offset..offset + slice.len()]);
        offset += slice.len();
    }
